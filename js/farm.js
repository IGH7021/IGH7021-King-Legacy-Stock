...
function farmRateLabel(service) {
  const qty = Number(service.unitsPerBaht) || 1;
  return `${fmtNum(qty)} ชิ้น / ฿1`;
}
...
function farmServiceCard(service) {
  ...
  return `<div class="glass-card rounded-2xl overflow-hidden flex flex-col" data-farm-id="${service.id}">
    ...
    <div class="p-3 flex-1 flex flex-col gap-1.5">
      ...
      <span class="text-[11px] w-fit px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">${service.category}</span>
...
function renderFarmOrders() {
  ...
  box.innerHTML = orders.map(order => `<div class="glass-card rounded-xl p-3 flex items-center gap-3">
    <div class="w-10 h-10 rounded-lg bg-emerald-500/15 flex items-center justify-center">🌾</div><div class="flex-1 min-w-0"><div class="flex justify-between gap-2"><span class="font-medium text-sm truncate">${order.serviceName}</span><span class="font-semibold text-sm text-emerald-400">${fmtBaht(order.money)}</span></div><div class="text-xs text-slate-400">${fmtNum(order.quantity)} ชิ้น • ${fmtDate(order.date)}${order.customer ? ` • ${order.customer}` : ""}${order.note ? ` • ${order.note}` : ""}</div></div><button data-farm-order-delete="${order.id}" class="text-xs px-2 py-1.5 rounded-lg bg-slate-700/60 hover:bg-rose-600/70" aria-label="ลบงาน">🗑️</button>
  </div>`).join("");
}
...
function openFarmServiceModal(id = null) {
  ...
  if (service) { form.name.value = service.name; form.category.value = service.category; form.rarity.value = service.rarity || "none"; form.rateQty.value = service.unitsPerBaht || 1; form.ratePrice.value = 1; form.description.value = service.description || ""; form.dataset.image = service.image || ""; if (service.image) { document.getElementById("farm-image-preview").src = service.image; document.getElementById("farm-image-preview-wrap").classList.remove("hidden"); document.getElementById("farm-image-placeholder").classList.add("hidden"); } }
...
function updateFarmOrderMoney() {
  ...
  document.getElementById("farm-order-money").value = quantity ? (quantity / (Number(service.unitsPerBaht) || 1)).toFixed(2) : "";
}
...
function handleFarmServiceSubmit(event) {
  ...
  const payload = { name: form.name.value.trim(), category: form.category.value.trim(), rarity: form.rarity.value, unitsPerBaht: quantity / price, description: form.description.value.trim(), image: form.dataset.image || null, updatedAt: Date.now() };
  ...
  saveState(); renderFarmServices(); closeModal("farm-service-modal"); toast("บันทึกงานฟาร์มแล้ว");
}
...
function handleFarmOrderSubmit(event) {
  ...
  if (!service || quantity <= 0 || money <= 0) { toast("กรุณากรอกจำนวนและราคาให้ถูกต้อง", "error"); return; }
  state.farmOrders.unshift({ id: genId("fo"), serviceId: service.id, serviceName: service.name, quantity, money: Math.round(money * 100) / 100, customer: form.customer.value.trim(), note: form.note.value.trim(), date: Date.now() });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("farm-services-grid").addEventListener("click", event => { const card = event.target.closest("[data-farm-id]"); const action = event.target.closest("[data-farm-action]")?.dataset.farmAction; if (!card || !action) return; const id = card.dataset.farmId; if (action === "order") openFarmOrderModal(id); else if (action === "edit") openFarmServiceModal(id); else if (action === "delete") { state.farmServices = state.farmServices.filter(item => item.id !== id); saveState(); renderFarmServices(); toast("ลบงานฟาร์มแล้ว"); } });
  document.getElementById("farm-orders-list").addEventListener("click", event => { const button = event.target.closest("[data-farm-order-delete]"); if (!button) return; state.farmOrders = state.farmOrders.filter(order => order.id !== button.dataset.farmOrderDelete); saveState(); renderFarmOrders(); toast("ลบประวัติงานแล้ว"); });
});
